...
OSM features whose canonical match key is already in the official key set skip both spatial filters (bbox and coverage). Scope exists to stop unmatched OSM from neighbouring Länder; it must not hide a key match when OSM geometry has drifted off the amtliche Fläche (for example Insel Lütje Hörn).
