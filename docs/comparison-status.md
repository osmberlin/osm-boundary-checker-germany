...
- `extract:official` (BKG branch) prepares `de-gemeinden-*/source/official.fgb` from shared VG25 cache using ARS-prefix filtering.
...
- Compare one area: `bun run compare -- --yes --area <folder>`
- Compare all discoverable areas: `bun run compare -- --yes` (or `bun run scripts/compare/compare-boundaries.ts -- --area <folder>` for a single area without the menu)
- Same compare engine: `bun run scripts/compare/compare-boundaries.ts -- --area <folder>`
- Regenerate OSM FlatGeobuf only (after PBF is present): `bun run extract:osm` (wizard) or `bun run --filter ./scripts extract:osm` (engine)

## Report app

- Typecheck: `bunx tsc --noEmit -p report/tsconfig.json`
- Build: `cd report && bun run build:with-runtime`
- Dev/preview server: `cd report && bun run dev` (or `bun run --filter report dev` from repo root)
