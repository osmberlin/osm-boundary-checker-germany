...
- WFS GeoJSON URL is in **`download.official`** (and duplicated under **`sources.official.sourceUrl`** for docs) in [config.jsonc](./config.jsonc). Run **`docker compose run --rm pipeline bun run berlin:download`** (or full **`docker compose run --rm pipeline bun run pipeline:nightly`**) to fetch and build **`source/official.fgb`**.
