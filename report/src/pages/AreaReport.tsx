...
import {
  type AreaTableSortKey,
  useAreaReportTableSort,
} from "../hooks/useAreaReportTableSort";
...
const ComparisonMapShell = lazy(
  () => import("../components/map/ComparisonMapShell"),
);

function unionMapBboxes(
  rows: AreaReportRow[],
...
export function AreaReport() {
  ...
  const { ref: chartRef, size: chartSize } =
    useMeasuredElementSize<HTMLDivElement>();
  ...
  const areaDisplayName =
    (areasIndexQuery.data?.summaries ?? []).find(
      (summary) => summary.area === areaKey,
    )?.displayName ??
    data?.area ??
    areaKey;
  const pageSourceHref =
    data?.sourceMetadata?.official?.sourceUrl?.trim() || null;
  ...
  const mainRows =
    data?.rows.filter(
      (r) => r.category === "matched" || r.category === "official_only",
    ) ?? [];
  const unmatchedRows = data ? normalizeUnmatchedRows(data) : [];
  const visibleMainRows = mainRows.filter((r) =>
    enabledCategories.includes(r.category),
  );
  const visibleUnmatchedRows = unmatchedRows.filter((r) =>
    enabledCategories.includes(r.category),
  );
  const visibleRows = [...visibleMainRows, ...visibleUnmatchedRows];

  const { sortedRows, sortBy, sortDir, setColumn } =
    useAreaReportTableSort(visibleRows);

  const catCounts = {
    matched: mainRows.filter((r) => r.category === "matched").length,
    official_only: mainRows.filter((r) => r.category === "official_only")
      .length,
    unmatched_osm: unmatchedRows.length,
  };

  const allMainOn =
    enabledSet.has("matched") && enabledSet.has("official_only");
  const mapAllowlist =
    !data || allMainOn ? null : visibleMainRows.map((r) => r.canonicalMatchKey);
  const unmatchedMapAllowlist =
    !data || enabledSet.has("unmatched_osm") ? null : [];
  ...
  const hausdorffMax = maxFiniteValue(
    sortedRows.map((row) => row.metrics?.hausdorffM),
  );
      ...
      >
        <StatBlocksRow className="mt-0">
          <LayerToggleStatBlock
            ...
            checked={
              catCounts.matched === 0 ? false : isCategoryEnabled("matched")
            }
          ...
          <LayerToggleStatBlock
            ...
            checked={
              catCounts.official_only === 0
                ? false
                : isCategoryEnabled("official_only")
            }
          ...
          <LayerToggleStatBlock
            ...
            checked={
              catCounts.unmatched_osm === 0
                ? false
                : isCategoryEnabled("unmatched_osm")
            }
                  ...
                  <ComparisonMapShell
                    sources={{
                      ...
                      unmatched: data.hasUnmatchedPmtiles
                        ? {
                            pmtilesUrl:
                              comparisonUnmatchedPmtilesMaplibreUrl(areaKey),
                            sourceLayer: data.tippecanoeLayer,
                            allowedFeatureIds: unmatchedMapAllowlist,
                            visible:
                              enabledSet.has("unmatched_osm") &&
                              mapLayers.showOsm,
                          }
                        : undefined,
                    }}
              ...
              >
                ...
                <td className="px-3 py-2 text-right text-slate-100 tabular-nums">
                  <MetricCellBar
                    ...
                    value={
                      row.metrics
                        ? formatDePercentPoints(row.metrics.areaDiffPct)
                        : EM_DASH
                    }
                  />
                </td>
...
function MetricCellBar({
  ...
  return (
    <div
      className="relative overflow-hidden rounded-sm px-1"
      style={metricBarStyle(ratio)}
    >
